...
public:
    int BGCOLOR = 0; // background and overlays
    int MAINCOLOR = 255; // drawings
    ...
    void drawEyes()
    {
        ...
        if (autoblinker)
        {
            ...
            {
                blink();
                blinktimer = millis() + (blinkInterval * 1000) + (local_random(blinkIntervalVariation) * 1000); // calculate next time for blinking
            }
        }
        ...
        if (idle)
        {
            ...
            {
                eyeLxNext = local_random(getScreenConstraint_X());
                eyeLyNext = local_random(getScreenConstraint_Y());
                idleAnimationTimer = millis() + (idleInterval * 1000) + (local_random(idleIntervalVariation) * 1000); // calculate next time for eyes repositioning
            }
        }
